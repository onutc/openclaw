---
summary: "Thread bound subagent sessions for thread-capable channels with plugin lifecycle hooks, routing, and config kill switches"
owner: "onutc"
status: "implemented"
...
This feature lets users interact with spawned subagents directly inside threads on channels that support thread functionality.
...
The implementation is split between channel agnostic core lifecycle hooks and channel-specific extension behavior. Discord is the first implemented thread-capable channel.

## Supported thread-capable channels

- Discord (implemented)
- Additional channels can use the same model once they add thread binding adapter support.
...
- Additional channel thread binding adapter implementations in this document.
...
- The first adapter (Discord) ships a thread binding manager with bind, unbind, TTL sweep, and persistence.
- Plugin hook lifecycle for subagents:
  - `subagent_spawning`
  - `subagent_spawned`
  - `subagent_delivery_target`
  - `subagent_ended`
- The Discord extension currently implements thread auto bind, delivery target override, and unbind on end.
- Text commands for manual control:
  - `/focus`
  - `/unfocus`
  - `/agents`
  - `/session ttl`
- Global and channel-scoped enablement and TTL controls, including a global kill switch.
...
Core (`src/agents/*`) does not directly depend on channel routing internals.

Core emits lifecycle intent through plugin hooks.

Discord extension (`extensions/discord/src/subagent-hooks.ts`) implements the first channel-specific behavior:
...
3. `subagent_delivery_target`
   ...
   - can redirect completion delivery to a bound thread origin (Discord today)
4. `subagent_ended`
...
For adapters that implement thread bindings, account override wins, then channel, then global session default, then built in fallback.

Current implementation details below use Discord keys:
...
When effective `enabled` is false for an account in the current Discord adapter:
...
- `thread: true` spawn path returns actionable error from the adapter hook layer
...
3. Current Discord adapter checks effective flags:
...
- `/focus <target>`
  - available on adapters with thread bindings (currently Discord)
...
- Active adapter preflight (Discord today) checks current thread id against thread binding manager.
...
- The active adapter extension (Discord today) returns bound thread origin when it can resolve one.
...
- `/session ttl` is currently implemented for Discord thread bindings.
...
- Use `channels.discord.threadBindings.enabled` and account overrides for selective enablement in the Discord adapter.
- Keep `spawnSubagentSessions` opt in for thread auto spawn behavior.
- Use TTL settings for automatic unfocus policy control.

This model keeps subagent lifecycle orchestration generic while giving thread-capable adapters a full thread bound interaction path. Discord is the first shipped implementation.
...
ACP remains a next step in that plan and is intentionally not implemented in this shipped Discord-first thread-bound flow.
